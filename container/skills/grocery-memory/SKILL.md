...
Analyze past Rohlik deliveries and maintain structured memory files in `/workspace/agent/grocery/` so the shopping planner always has up-to-date knowledge about the household.
...
All files live in `/workspace/agent/grocery/`. Create the directory if it doesn't exist.
